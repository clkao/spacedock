...
- DONE: Independently reproduce the fence task-prompt (recorded-Launch oracles, built from the worktree) AND map the captain's REAL ps lines to `spacedock claude/codex` invocations, confirming emitted argv.
  Authored an INDEPENDENT oracle file `validation_ps_parity_test.go` (not the implementer's) compiled against the worktree's `internal/cli`, driving the real `runClaude`/`runCodex` via the `fakeHost` seam; 5/5 passed. Mapped each real ps line: claude `--plugin-dir /Users/clkao/git/spacedock --safehouse -- "Triage…"` → `safehouse --trust-workdir-config -- claude --dangerously-skip-permissions --agent spacedock:first-officer --plugin-dir … <base+" Triage…">`; two-plugin-dir plain launch (order preserved); codex `--safehouse resume <uuid>` → bypass + `resume <uuid>` with NO prompt; codex `--safehouse-enable=ssh -- "Engage ops…"` → `--enable=ssh` pre-`--` + `<codexBase+" Engage ops…">`; bare claude → base only (no trailing space). Fence semantics confirmed: `... -- "task"` → base+" "+task; bare → base; claude resume-family (`-r`/`--resume`/`--continue`/`-c`) and codex leading `resume` → NO prompt. Removed the validation-only file after the run (worktree clean; not part of the deliverable).
- DONE: Independently verify `--plugin-dir` (one+/order) forwards verbatim AND relaxes the gate for BOTH hosts; a failing manifest WITHOUT `--plugin-dir`/`--skip-contract-check` still rc≠0 no-launch. Plus cycle-2 knobs: TranslateFlags, wrap-trigger OR, imply-on.
  `TestPluginDirRelaxesGate` (claude/codex relax on a `tooOldBinaryManifest`; `no-plugin-dir-still-fails-fast` asserts rc≠0 + `launchedArg==nil`) and my two-plugin-dir oracle confirm verbatim/order forwarding. Cycle-2: `TestTranslateFlags` (enable comma→repeated `--enable=`, add-dirs/add-dirs-ro, order) + `TestTranslateFlagsUnknownKeyErrors` (AC-8 hard-error); OR wrap-trigger `Present(dir)||forceSafehouse||len(safehouseFlags)>0` exercised by AC-4/5/6 forced-wrap oracles in no-profile dirs; imply-on confirmed (`TestKnobImpliesSandboxOnNoProfile` claude+codex). All exercise+observe, none prose-grep.
- DONE: Gates green with REAL captured exit codes; AC cross-check every LP-AC-1..3 + AC-1..8 has reproduced behavioral evidence.
  `go test ./...` rc=0 (466 passed); `go test -race ./...` rc=0; `gofmt -l .` empty (rc=0); `go vet ./...` rc=0 — all captured live in the worktree. Ran every AC oracle with `-v` (raw, un-summarized): AC-1..8 + LP-AC-1..3 + `TestSplitFrontDoorArgs` table all `--- PASS`, no `t.Skip` in launch-parity scope. Each AC's proof exercises the launch and asserts recorded argv/rc — applied the behavioral-proof bar, found no tautological/prose-grep AC. AC-3's namespace-seam claim is behaviorally backed (translator sees no `--safehouse-` token; dispatcher strips the prefix).

### Summary
Recommend **PASSED**. Independently reproduced the fence task-prompt grammar and mapped all the captain's real `ps` lines (claude `--plugin-dir`+task, codex resume, codex `--enable`+task, bare→base) to `spacedock claude/codex` invocations with matching emitted argv, using a self-authored oracle compiled against the worktree (5/5 green, then removed). Verified `--plugin-dir` verbatim/multiplicity forwarding + gate-relax for both hosts and the failing-manifest-without-plugin-dir no-launch regression, plus the cycle-2 knobs (TranslateFlags comma-split/add-dirs/unknown-key-error, OR wrap-trigger, imply-on). All gates green with real captured exit codes (test 466 passed, -race, gofmt clean, vet clean). Every LP-AC-1..3 and AC-1..8 has exercise-and-observe behavioral evidence — no prose-grep or tautological proofs. AC live-smoke against real safehouse/codex deferred to captain (we are inside safehouse). No material issues found (validator-only recalibration; no parallel staff audit).
