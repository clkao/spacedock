...
completed: 2026-05-31T21:32:52Z
verdict: PASSED
worktree: 
issue:
---
...
- DONE: A0 (AC-0) — absolutize `workflowDir` once at the top of `runBuild` so EVERY downstream join inherits an absolute base; relative-`--workflow-dir` worktree dispatch-build test, RED before / GREEN after; fetch-line spelling parity holds.
  build.go:142 inserts `filepath.Abs(workflowDir)` before the README join; the emitted state-commit `git -C` and the fetch-line `--workflow-dir` are now absolute. RED-first test `TestStateCommitPathAbsoluteFromRelativeWorkflowDir` (build_statecommit_test.go) asserts `filepath.IsAbs` on the emitted `git -C` path; proven RED by removing the A0 block (emits `wf/state-checkout`) then GREEN restored. Parity cross-product (incl. the fetch line) stays green. Commit ff98605.
- DONE: A1 (AC-1) — emit `state_backend`/`definition_dir`/`entity_dir`/`entity_dir_present` in `status --boot --json` appended AFTER `team_state`; STATE_BACKEND text banner; split + single-root fixtures.
  bootData gained the four fields (boot.go), populated in gatherBoot from definitionDir/entityDir (split-root when they diverge) + `os.Stat` for entity_dir_present; bootJSON appends them after team_state; bootJSONKeys updated; printBoot adds the STATE_BACKEND banner. json_boot_test.go: TestBootJSONStructure now pins single-root values; new TestBootJSONStateBackendSplitRoot (split, entity_dir absolute + ends in .spacedock-state + diverges) and TestBootJSONStateBackendEntityDirAbsent (entity_dir_present false). The native-only banner is stripped from the oracle parity normalizers (harness_test.go stripStateBackend, wired into native_read_test.go + zz_independent_parity_test.go) — full status oracle parity green.
- DONE: A2 (AC-2) — split-root-vs-single-root decision-rule prose in commission SKILL.md §2a + static skill-text test.
  SKILL.md §2a carries the `state: .spacedock-state` usage, the "embedded in a code repo whose PRs you care about" trigger, and the single-root "omit `state:`" guidance. TestCommissionStateBackendDecisionRule (skills/integration/skill_text_test.go) asserts all three fragments; RED before the prose, GREEN after.
- DONE: A3 (AC-3) — single-root negative dispatch-build test, both worktree and non-worktree branches.
  TestSingleRootNoStateCommitGuidance (build_statecommit_test.go) asserts a no-`state:` workflow emits NO `This workflow is split-root` sentence and NO `git -C` command on both branches; proven to bite by forcing `splitRoot := true` (3/3 fail) then reverting (3/3 pass).
- DONE: Full gates green with REAL captured exit codes from inside the worktree.
  `go test ./...` exit 1 — sole failure is the pre-existing env-only `TestCodexResolveManifestAgainstInstalledHost` (sandbox blocks `~/.codex/config.toml`, untouched by this change); every changed package (dispatch, status, contract, ensigncycle, release, safehouse, skills/integration) is `ok`. `go vet ./...` exit 0, gofmt clean.

### Summary

Phase A (A0–A3) is implemented and committed on the worktree branch (ff98605). A0 absolutizes `workflowDir` once in runBuild, making the emitted state-commit `git -C` and the fetch-line `--workflow-dir` cwd-independent — retiring the FO's hand-injection of absolute state paths. A1 surfaces the backend in `status --boot` (JSON keys appended after team_state to preserve the FO's key-order contract, plus a text banner) including the `entity_dir_present` absent-checkout diagnostic. A2 adds the commission decision rule; A3 pins the single-root no-leak negative. The only judgment call was the text STATE_BACKEND banner, which is a deliberate native-vs-oracle divergence handled with a documented strip in the parity normalizers (the same pattern the dispatch fetch-line and state-commit guidance already use). AC-4/AC-5 (remote determination + 2nd-host resume) are intentionally NOT here — they are the split Phase-B entity.

## Stage Report: validation

- DONE: Reproduce A0 (AC-0) independently — RED/GREEN proof of the absolutize fix
  GREEN baseline: `TestStateCommitPathAbsoluteFromRelativeWorkflowDir` passes (7/7 dispatch state-commit tests). RED proof: commented out the `filepath.Abs(workflowDir)` block (build.go:148-150) → test FAILS, emitted `git -C ../../../../tmp/.../wf/state-checkout …` relative (and the fetch-line `--workflow-dir` also went relative, confirming A0 fixes both halves). Restored → GREEN; worktree `git diff` byte-clean vs HEAD.
- DONE: Reproduce A3 (AC-3) independently — single-root no-leak, BOTH branches
  GREEN baseline: `TestSingleRootNoStateCommitGuidance/{worktree,nonworktree}-stage` pass. RED proof: forced `splitRoot := true` (build.go:235) → BOTH subtests FAIL on both the `This workflow is split-root` sentence AND the `git -C` command (4 assertions). Restored → GREEN; worktree byte-clean.
- DONE: A1 (AC-1) boot surfacing — split / single-root / absent-checkout fixtures + key-order
  `TestBootJSONStructure` (single-root: state_backend single-root, definition_dir==entity_dir, absolute, present true), `TestBootJSONStateBackendSplitRoot` (entity_dir absolute, ends in .spacedock-state, diverges), `TestBootJSONStateBackendEntityDirAbsent` (entity_dir_present false) all PASS. The four keys (state_backend, definition_dir, entity_dir, entity_dir_present) are appended AFTER team_state in bootJSON and pinned in bootJSONKeys; key-order assertion green.
- DONE: CRUX — STATE_BACKEND parity-strip subtracts ONLY the banner, masks no real divergence (k6-strip lesson)
  Verified `stateBackendLineRe` (`STATE_BACKEND: [^\n]*\n`) is line-surgical: `[^\n]*` cannot cross a newline (ORPHANS/other lines survive). DECISIVE test: planted a PROBE-DIVERGENCE in the native TEAM_STATE hint → `TestNativeBootMatchesOracle` STILL FAILS despite the strip, proving the strip is narrow and does not blanket-mask native-vs-oracle divergence. Reverted → green.
- DONE: A2 (AC-2) commission decision-rule prose + static test
  SKILL.md §2a carries all three fragments (`state: .spacedock-state`, the "embedded in a code repo whose PRs you care about" trigger, the single-root "omit `state:`" guidance); `TestCommissionStateBackendDecisionRule` PASS.
- DONE: A2-collateral — absolutizing workflowDir broke no downstream consumer (fetch-line now absolute)
  `filepath.Abs` verified idempotent on already-absolute, cleans `..`/`.`, never errors; A0 changes only the fetch-line SPELLING (same resolved target). Full dispatch parity green: `TestBuildParityCrossProduct` (8 rows, split/single × folder/flat × worktree/nonworktree × team/bare), `TestBuildAbsoluteWorktreeParity`, `TestShowStageDefSeparatorParity` all PASS.
- DONE: Full gates with REAL captured exit codes from inside the worktree
  `go test ./...` exit 1 — sole failure `TestCodexResolveManifestAgainstInstalledHost` (internal/cli, env-only: sandbox blocks codex PATH/config; ff98605 touches NO cli files — confirmed pre-existing, not a regression). Every changed package ok (dispatch, status, contract, ensigncycle, release, safehouse, skills/integration). `go vet ./...` exit 0; `gofmt -l` clean.

### Summary

PASSED. Independently reproduced A0 and A3 RED/GREEN by disabling the absolutize block and forcing splitRoot=true — both proofs bite exactly as the implementation claims, and the worktree returns byte-clean after each revert. A1's four state-backend keys are correctly appended after team_state (FO key-order contract preserved) and the split/single/absent fixtures all pass. The CRUX is satisfied: the STATE_BACKEND parity strip is line-surgical and a planted non-banner divergence still fails parity, so it does not mask the k6-style native/oracle drift. A0's fetch-line spelling change broke no downstream consumer — full dispatch cross-product parity is green and filepath.Abs is well-behaved on all edges. The only `./...` failure is the pre-existing env-only codex test, explicitly out of scope. AC-4/AC-5 correctly absent (split Phase-B entity).
