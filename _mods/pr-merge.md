...
Compute the audit-link inputs first: short SHA via `git -C {worktree} rev-parse --short HEAD` (if it exits non-zero — no commits, detached HEAD — substitute the literal string `next` and report the fallback to the captain); owner/repo via `gh repo view --json nameWithOwner --jq '.nameWithOwner'`; short entity-id slot via `spacedock status --workflow-dir docs/dev --short-id {slug}` (shortest-unique-prefix for sd-b32 workflows, matching the status table's ID column).
