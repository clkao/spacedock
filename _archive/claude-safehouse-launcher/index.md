...
- DONE: Independently reproduce AC-1/AC-2/AC-5 argv oracles (build from the worktree, drive runClaude via the recorded-Launch fake).
  Wrote a throwaway oracle (internal/cli/validation_independent_test.go, since removed) transcribing the canonical argv from the spec text — not copied from the implementer's tests — and built/ran from the worktree: TestVAC1/VAC2/VAC5 all PASS. AC-1 argv = `[safehouse --trust-workdir-config -- claude --dangerously-skip-permissions --agent spacedock:first-officer --foo <bootstrap-prompt>]`; AC-2 = `[claude --agent spacedock:first-officer --foo <bootstrap-prompt>]` (no `safehouse` token, no `--dangerously-skip-permissions`); AC-5 (`--resume`) drops the bootstrap prompt. In-tree TestClaudeSafehousePresentWrapsArgv / NoSafehouseLaunchesPlain / ResumeSuppressesBootstrapPrompt also PASS; `.safehouse`-as-directory detected identically (TestClaudeSafehouseDirDetectedLikeFile / TestVAC1 dir variant).
- DONE: Independently verify AC-3 (plugin-gate short-circuit) and AC-4 (binary absent → pinned hint), and that Present+Available+Wrap is the shared inner-argv-agnostic seam.
  TestVAC3 + in-tree TestClaudePluginGateShortCircuitsBeforeSafehouse PASS: with `.safehouse` present AND binary absent AND no plugin, rc≠0, Launch never fired, plugin remedy on stderr, NO `.safehouse profile` hint. Mutation check: replaced the gate `if !gateHost(...) {return 1}` with a result-ignoring call — both AC-3 oracles FAILED, then restored — proving the ordering invariant is genuinely guarded, not vacuous. TestVAC4 + TestClaudeSafehousePresentButBinaryMissing PASS: rc≠0, pinned `.safehouse profile` hint, no Launch. internal/safehouse exposes Present/Available/Wrap, all inner-argv-agnostic (Wrap composes prefix only); 7 unit tests PASS.
- DONE: Gates green with REAL captured exit codes; AC cross-check; AC-6 recorded CAPTAIN-CONFIRMED PASSED (not re-run).
  `go test ./...` → 404 passed, RC=0; `go test -race ./...` → 404 passed, RC=0; `gofmt -l .` → no files, RC=0; `go vet ./...` → no issues, RC=0. Every **AC-N** has reproduced evidence above. AC-6 CAPTAIN-CONFIRMED PASSED: captain ran the canonical argv outside the sandbox and observed claude's own `Usage: claude [options] [command] [prompt]` help (not a safehouse/claude flag error) — confirming safehouse consumed `--trust-workdir-config` and `--` handed the remainder to claude. The AC-6 smoke script (docs/dev/_evidence/.../ac6-live-safehouse-smoke.sh:30) runs that exact argv; canonical argv is locked and consistent across AC-1 test, AC-6 script, and captain confirmation. Did NOT re-run real safehouse (we run inside it).
...
## Stage Report: validation (cycle 2 — resume-family widening re-validation)

- DONE: Reproduce AC-5 (resume FAMILY): the bootstrap prompt is suppressed for each of `--resume`, `--resume=<id>`, `-r`, `--continue`, `-c`, and is APPENDED for a normal non-resume arg; confirm AC-1/AC-2 canonical argv are unchanged by the widening.
  `TestClaudeResumeFamilySuppressesBootstrapPrompt` — 5 subtests (resume / resume-equals / resume-short / continue / continue-short) all PASS; each argv forwards the operator's resume arg verbatim and carries NO bootstrap-prompt token. AC-1 positive (`TestClaudeSafehousePresentWrapsArgv`, non-resume `--foo`) and AC-2 (`TestClaudeNoSafehouseLaunchesPlain`) both PASS — prompt still appended on the non-resume path. `git show 04c7d30 -- internal/cli/frontdoor.go` confirms the widening touched ONLY `containsResume` (+`strings` import); the canonical launch-argv assembly (`runClaude` lines 98-115) is byte-for-byte unchanged, so AC-1/AC-2 argv are unaffected.
- DONE: Adversarially confirm the widened matcher is NOT over-broad: a forwarded arg that merely resembles a resume flag must NOT spuriously suppress the prompt — drove negative cases and observed the prompt still appended.
  Throwaway `TestVNegResumeMatcherNotOverBroad` (added, run, removed) drove `--resume-foo`, `--restore`, `-rc`, bare `resume`, `--continued` — ALL kept the bootstrap prompt (matcher not over-matched); genuine `-r` correctly suppressed it. Mutation check: widening the `strings.HasPrefix(a, "--resume=")` guard to `"--resume"` made `--resume-foo` over-match and the negative oracle FAILED — proving it genuinely guards over-broadness, then restored (frontdoor.go `git diff` clean, `--resume=` prefix intact). `containsResume` uses an exact-match switch + a `--resume=` prefix only, so resume-resembling args do not over-match.
- DONE: Gates green with REAL captured exit codes; AC cross-check every **AC-N**; AC-6 CAPTAIN-CONFIRMED PASSED, noted unchanged by this widening (not re-run).
  `go test ./...` → 409 passed, RC=0; `go test -race ./...` → 409 passed, RC=0; `gofmt -l .` → no files, RC=0; `go vet ./...` → no issues, RC=0 (captured after removing the throwaway test; tree clean, `git status --short` empty). AC-1/AC-2 reproduced above (argv unchanged); AC-3 (plugin-gate short-circuit) and AC-4 (binary-absent pinned hint) carry forward from cycle-1 validation, unaffected by a prompt-predicate-only change; AC-5 reproduced over the full family. AC-6 CAPTAIN-CONFIRMED PASSED earlier and is NOT re-run — the widening changes only the prompt-suppression predicate, not the canonical launch argv (which AC-6 gates), so AC-6 remains valid. Did NOT re-run real safehouse (we run inside it).

### Summary

PASSED. The resume-family widening (commit 04c7d30) is correct and correctly scoped. AC-5 now suppresses the bootstrap prompt across the full session-resume family (`--resume`, `--resume=<id>`, `-r`, `--continue`, `-c`) — all five forms reproduced green — while AC-1/AC-2 canonical argv are provably unchanged (the diff touched only `containsResume`). Adversarially confirmed the matcher is NOT over-broad: resume-resembling args (`--resume-foo`, `--restore`, `-rc`, bare `resume`, `--continued`) still get the prompt appended, and a deliberate over-broadening mutation makes the negative oracle fail (so the guard is real, not vacuous). All four gates pass with captured RC=0 (test 409, race 409, gofmt clean, vet clean). AC-6 is recorded CAPTAIN-CONFIRMED PASSED and is correctly NOT re-run, since the widening does not change the canonical launch argv it gates. Recommendation: PASSED.
