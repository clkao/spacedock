...
- DONE: Add ONE integration test that builds the real .spacedock-state/README.md -> ../README.md symlink layout in a temp tree (symlink created at runtime via os.Symlink; include the misdetection trap: a folder-form entity with reports/ideation.md carrying real frontmatter) and drives `spacedock status --workflow-dir <state-dir>` to assert: entities render through the symlinked README (AC-1); folder-form reports/artifacts are NOT misdetected — active slug set is exactly {add-login, refactor-dispatch, seed-archive}, 3 data rows (AC-2); --archive moves the whole folder under .spacedock-state/_archive carrying its reports/ subtree, removed from active and present in --archived (AC-3); all assertions go through the `spacedock status` argv surface, not internal functions (AC-4).
  `internal/status/symlink_profile_test.go` `TestSymlinkStateProfile` — builds the layout, `os.Symlink("../README.md", ...)` at runtime, asserts ordered slug set {add-login,refactor-dispatch,seed-archive}=3 rows, `--next` resolves (no "no stages block"), `--archive seed-archive` moves the folder + `reports/` subtree to `_archive/` with an `archived:` stamp, gone from default table, present in `--archived`. All via `runLauncher` (the VendorRunner = `spacedock status` argv seam). Non-vacuity verified: breaking the symlink target makes the default-table assertion fail (README unresolved → sequential fallback → "missing required id"). worktree commit 7a423ca.
- DONE: Fixtures under internal/status/testdata/symlink-profile/ (checked-in plain files; symlink materialized at runtime). NO product code change — this is a layout+integration proof. Place the test so it does not collide with sibling work in internal/status (a distinct filename / _test package is fine; or internal/cli if the launcher route is reached more naturally there).
  Seven checked-in fixtures: README.md (id-style: slug, 4-stage block), add-login.md (flat), refactor-dispatch/{index.md,reports/ideation.md[+frontmatter trap],artifacts/notes.md}, seed-archive/{index.md,reports/ideation.md}. Symlink created at runtime, not checked in. Zero product code touched — only the new `symlink_profile_test.go` (distinct filename) + new `testdata/symlink-profile/` dir; file-disjoint from the sibling native-go-status work in `internal/status/status.go`.
- DONE: go test ./... + -race + gofmt -l ./cmd ./internal + go vet green; the test drives the REAL launcher binary (no mocks).
  `go test ./...` 44 passed; `go test ./... -race` 44 passed; `gofmt -l ./cmd ./internal` clean (no output); `go vet ./...` no issues. The test drives the real vendored runner in-process via `runLauncher` — no mocks.

### Summary

Proved the `.spacedock-state/README.md -> ../README.md` compatibility bridge with one integration test (`TestSymlinkStateProfile`) and seven checked-in fixtures, no product code change. The test builds the symlink layout in a temp tree, materializes the symlink at runtime, and drives the unmodified Stage 2 status runner through the `spacedock status` argv surface (`runLauncher`/`VendorRunner`) to assert all four ACs: render-through-symlink + stage ordering (AC-1), folder-form non-misdetection with a frontmatter-bearing nested report as the trap (AC-2), whole-folder archive carrying the `reports/` subtree (AC-3), and every assertion through the command surface (AC-4). Chose `id-style: slug` so entities need no pinned numeric id; this also makes the test self-validating — a broken symlink drops README resolution to the sequential fallback and fails with "missing required id", confirming the test genuinely depends on the symlink. Kept additions file-disjoint from the concurrent native-go-status ensign (new test file + testdata dir only). All gates green: go test ./... (44), -race (44), gofmt, go vet.

## Stage Report: validation

- DONE: Independently reproduce all four ACs: run TestSymlinkStateProfile AND manually build a .spacedock-state/README.md -> ../README.md layout in a temp dir and drive `spacedock status --workflow-dir <state>` yourself to confirm: entities render through the symlinked README + stage ordering (AC-1); folder-form reports/artifacts NOT misdetected — exactly {add-login, refactor-dispatch, seed-archive}, 3 rows (AC-2); --archive moves the whole folder under _archive carrying reports/, gone from active, present in --archived (AC-3); all via the spacedock status argv surface (AC-4). Confirm the test is non-vacuous (breaking the symlink target makes it fail).
  `go test ./internal/status -run TestSymlinkStateProfile -v` -> RUN + PASS (0.17s), not skipped. Then built `./cmd/spacedock` and hand-built the symlink layout in a temp dir (real `ln -s ../README.md`), driving the actual binary: default table exits 0 with exactly rows {refactor-dispatch, add-login, seed-archive} ordered ideation<implementation<review (AC-1 render+order), no reports/artifacts/ideation row despite refactor-dispatch/reports/ideation.md carrying `id: trap-not-an-entity` frontmatter (AC-2 non-misdetection); `--next` resolves CURRENT->NEXT from the stages block read through the symlink, no "no stages block" (AC-1 README-through-symlink); `--archive seed-archive` -> active seed-archive/ gone, `_archive/seed-archive/index.md` stamped `archived: 2026-05-30T06:16:47Z`, `_archive/seed-archive/reports/ideation.md` carried, default table omits it, `--archived` includes it (AC-3); every assertion via the `spacedock status` argv surface (AC-4). Non-vacuity: a dangling `../README.md` symlink makes the default table exit 1 (`missing required id`, README unresolved -> sequential fallback) and `--next` exit 1 (`README.md has no stages block`) — the test genuinely depends on the symlink resolving.
- DONE: Confirm NO product code changed (only the new test file + testdata); re-run go test ./..., -race, gofmt, go vet — all green.
  `git diff --name-only 83c7349 7a423ca` lists only `internal/status/symlink_profile_test.go` + 7 files under `internal/status/testdata/symlink-profile/`; grep for any non-`_test.go`/non-`testdata` path is empty. `go test ./...` ok (cli, status, integration); `go test ./... -race` ok; `gofmt -l ./cmd ./internal` clean (exit 0, no output); `go vet ./...` clean (no output). Worktree `git status` clean after validation.
- DONE: Give a PASSED or REJECTED recommendation backed by reproduced evidence; if REJECTED, name the concrete gap.
  PASSED. All four ACs reproduced independently against the real `spacedock status` binary on a hand-built symlink layout, the committed `TestSymlinkStateProfile` runs (not skipped) and passes, non-vacuity confirmed by breaking the symlink target, no product code changed, and all four gates green.

### Summary

Independent validation of the symlink-state-profile proof: PASSED. Reproduced every AC two ways — the committed `TestSymlinkStateProfile` (RUN+PASS, 0.17s, drives the real in-process `VendorRunner` that execs the 2546-line vendored Python oracle, no mocks) and a fully independent rebuild where I hand-constructed the `.spacedock-state/README.md -> ../README.md` layout in a temp dir and drove the freshly-built `spacedock status` binary. The default table renders exactly {refactor-dispatch, add-login, seed-archive} ordered by the stages block read through the symlink; the folder entities' frontmatter-bearing `reports/ideation.md` is not misdetected; `--archive` relocates the whole `seed-archive/` folder (with its `reports/` subtree) under `_archive/` with an `archived:` stamp, gone from active and present in `--archived`. Non-vacuity holds: a dangling symlink target makes both the default table and `--next` fail. The commit touches only the test file plus 7 testdata fixtures — no product code — and `go test ./...`, `-race`, `gofmt -l ./cmd ./internal`, and `go vet ./...` are all green. Note: the `claude-team show-stage-def` fetch command in the dispatch failed (exit 127, not on PATH), but the full stage spec/ACs were present in the entity file, so validation ran on complete context, not empty context.
