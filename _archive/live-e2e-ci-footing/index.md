...
completed: 2026-05-31T23:52:43Z
verdict: PASSED
worktree: 
issue:
...
- DONE: Generalize the live test's stage-report-heading assertion. live_test.go reused the package-level backlog-specific `stageReportHeading` (`^## Stage Report: backlog$`), but the live FO drives the entity to the TERMINAL `done` stage so the ensign writes `## Stage Report: done` — the backlog regex never matched (CI sonnet+opus both completed+archived the cycle yet failed `entity missing anchored stage-report heading`). Replaced the heading check with a stage-agnostic LOCAL regex matching ANY named stage; still goes RED when there is no `## Stage Report:` at all.
  Added `liveStageReportHeading = regexp.MustCompile((?m)^## Stage Report: \S)` to liveassert_test.go (default tags, beside the other live end-state regexes); live_test.go's (a)-check now calls it. `\S` after the colon requires a named stage and rejects an entity with no stage report (the incomplete-cycle go-red shape). doneMarker + `### Summary` + !checkboxBullet + verdictSet + frontmatterField(status:done) + someCommitNamesOnly(archive-aware) all UNCHANGED. Code commit dffa4cd.
- DONE: Update any liveassert helper/unit test that pins the backlog heading to the generalized form; keep the offline unit tests green.
  No existing unit test pinned the heading (the backlog regex was only ever exercised by the deterministic skeleton's own cycle_test.go, which is untouched). Added a NEW offline unit test `TestLiveStageReportHeading` (liveassert_unit_test.go, default tags): asserts the regex matches `## Stage Report: done`/`backlog`/`implementation` AND does NOT match an entity with no `## Stage Report:` section nor a colon-with-no-stage heading — the go-red discipline pinned offline, no live model.
- DONE: Offline gates green with REAL captured exit codes (go build ./..., go test ./... [live excluded], go vet, gofmt -l); the live test compiles under -tags live and skips cleanly with no creds. Do NOT run the nested live cycle.
  `go build ./...` exit 0; `go vet ./...` + `go vet -tags live ./internal/ensigncycle/` exit 0; `gofmt -l internal/ensigncycle/` clean (0 files). Full `go test ./...` = ensigncycle package GREEN (incl. new TestLiveStageReportHeading PASS); sole repo failure is the PRE-EXISTING `TestCodexResolveManifestAgainstInstalledHost` cli env artifact (`~/.codex/config.toml: Operation not permitted`) — diff touches only the 3 ensigncycle files, cli untouched, not a regression. Build-tag isolation holds: `TestLiveEnsignCycle` present ONLY under `-tags live -list`, absent from default `-list`; the new unit test is in BOTH (correct — it's default-tag). Compile-only `-tags live -run '^$'` exit 0. Clean SKIP proven: `env -u ANTHROPIC_API_KEY -u CLAUDE_CODE_OAUTH_TOKEN HOME=<fresh>` → `--- SKIP` before any claude exec. Nested live cycle NOT run (FO cherry-picks onto the PR branch + re-dispatches the CI).

### Summary

Applied the cycle-4 one-regex fix: generalized the live test's stage-report-heading anchor from the backlog-specific `stageReportHeading` (`^## Stage Report: backlog$`) to a stage-agnostic LOCAL `liveStageReportHeading` (`^## Stage Report: \S`). The CI run proved the whole infra (offline passed, both CI-E2E/CI-E2E-OPUS gates approved, sonnet $1.38 + opus both ran the real model and COMPLETED dispatch→ensign→done→archive); both live legs failed on the single assertion `entity missing anchored stage-report heading` because the live FO drives the entity to the TERMINAL `done` stage, so the finishing ensign writes `## Stage Report: done` — which the inherited backlog regex never matched. `\S` after the colon requires a named stage (matching any of backlog/implementation/done) while still rejecting an entity with no stage report, so the test stays RED on an incomplete cycle. Added the offline unit test `TestLiveStageReportHeading` pinning that go-red discipline ($0, no live model). Everything CLEARED stays untouched: the auth/HOME-isolation fix, the sonnet+opus matrix, the archive-aware locate, the verdict-set relaxation, and the gate/trigger/permissions. All offline gates green with real captured exit codes (build/vet/gofmt/test/build-tag isolation/compile-under-tag/clean-skip); the lone `go test ./...` failure is the pre-existing codex-env cli artifact. Code commit dffa4cd on spacedock-ensign/live-e2e-ci-footing — the FO cherry-picks it onto the clean PR branch (live-e2e-ci-pr) and re-dispatches the CI.

## Stage Report: implementation (cycle 6)

- DONE: Add a `pull_request` trigger to `.github/workflows/runtime-live-e2e.yml` `on:` (KEEP workflow_dispatch): `pull_request: branches: [next]`. MUST be `pull_request`, NOT `pull_request_target`.
  `on:` now carries both keys (yq `.on | keys` → `[workflow_dispatch, pull_request]`); `.on.pull_request` → `branches: [next]`. NOT pull_request_target — pull_request withholds repo secrets from FORK PRs, so a fork's code cannot exfiltrate ANTHROPIC_API_KEY (secret simply absent). The workflow_dispatch inputs (claude_version/effort) are UNCHANGED.
- DONE: Label-gate the secret-bearing `claude-live` job: `if: ${{ github.event_name == 'workflow_dispatch' || contains(github.event.pull_request.labels.*.name, 'run-live-e2e') }}`. Keep `offline` UNCONDITIONAL; keep the `environment:` required-reviewer gate on claude-live.
  `.jobs.claude-live.if` == the exact gate verbatim; `.jobs.offline.if` == null (unconditional per-PR gate, no secrets). The per-variant `environment.name: ${{ matrix.environment }}` (sonnet→CI-E2E, opus→CI-E2E-OPUS) and `ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}` are UNCHANGED. Net gates for a live PR run: same-repo-or-no-secrets + the run-live-e2e label + the CI-E2E* deployment approval. Updated the SECURITY MODEL header comment to describe the new trigger (was "workflow_dispatch-only").
- DONE: Verify gofmt/go vet/offline `go test ./...` green with real exit codes; workflow YAML valid (actionlint); the `workflow_dispatch` matrix path UNCHANGED; do NOT touch auth fix, sonnet+opus matrix, assertions, or the ANTHROPIC_API_KEY wiring.
  `gofmt -l .` clean (0 files); `go vet ./...` clean; `go build ./...` exit 0; `go test ./internal/ensigncycle/` exit 0 (the only package touched by behavior — but this diff is YAML-only, no Go changed). Full `go test ./...` = 505 passed / 1 failed where the SOLE failure is the PRE-EXISTING `TestCodexResolveManifestAgainstInstalledHost` cli env artifact (`~/.codex/config.toml: Operation not permitted`) — my diff is YAML-only (`git diff --stat` = 1 file, the workflow; 27 insertions / 11 deletions), so the cli test cannot be a regression. actionlint (real, `go run github.com/rhysd/actionlint/...`) exit 0, NO findings. workflow_dispatch matrix path (matrix.include sonnet/opus, environment, secret, inputs) untouched per yq; the live-test Go files, auth fix, and assertions were NOT touched.

### Summary

Folded live-on-PR into the existing `runtime-live-e2e.yml` per the captain's directive: added a `pull_request: branches: [next]` trigger alongside the kept `workflow_dispatch`, and gated the secret-bearing `claude-live` matrix job behind an explicit `run-live-e2e` label (`if: workflow_dispatch || contains(labels.*.name, 'run-live-e2e')`). It is `pull_request`, NOT `pull_request_target` — so GitHub withholds repo secrets from fork PRs (the fork-exfil vector the audit flagged stays closed) while same-repo PRs get the secret; the per-variant CI-E2E/CI-E2E-OPUS environment approval and ANTHROPIC_API_KEY wiring are untouched. The `offline` gate stays unconditional (per-PR, no secrets). Net gates for a live PR run: same-repo-or-no-secrets + the label + the deployment approval. The change is localized to the workflow `on:` block, the claude-live `if:`, and the SECURITY MODEL header comment (YAML-only, 27/11 diff) — no Go, no auth-fix, no matrix, no assertion, no secret-wiring changes. All offline gates green with real captured exit codes (gofmt/vet/build/ensigncycle test/actionlint); the lone full-suite failure is the pre-existing codex-env cli artifact, not a regression. This is a SECURITY-SURFACE change on the trigger — the FO re-runs the adversarial security audit on the new pull_request trigger before merge. Did NOT run any live cycle (the FO cherry-picks onto live-e2e-ci-pr, re-audits, re-runs sonnet, merges). Code commit 428b70e on spacedock-ensign/live-e2e-ci-footing.
