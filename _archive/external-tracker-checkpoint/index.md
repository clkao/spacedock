...
- DONE: Independently reproduce evidence for ALL six ACs (do not rubber-stamp the impl report): AC-1 status read renders issue/source and does not require them, AC-2 --set preserves them, AC-3 the new TestNativeFolderEntityReportAppendPreservesTracker, AC-4 --archive preserves them, AC-5 static no tracker branching, AC-6 static v1 DECISION present.
  AC-1: `TestIndReadFlagsSeq` (12 read flags: default/fields/all-fields/validate/boot/...) diffed vs the independent oracle on `001-alpha.md` carrying `source: roadmap`/`issue: ENG-101`/`tracker-url:` while siblings 002-005 lack `issue` — parsed-not-stripped, not required; `parseFrontmatter` builds a generic key map (frontmatter.go:91), no fixed struct. AC-2: `TestIndMutationSeq/set-unrelated-keeps-unknown` (`--set 001 status=ideation`, full-file diff vs oracle) + `TestNativeUnknownFieldPreservation/set-unrelated-field` (byte-for-byte). AC-3: `TestNativeFolderEntityReportAppendPreservesTracker` PASS — status parity vs oracle, `strings.Count(out,"070-tracker")==1`, `source: kata`/`issue: kata:task-xyz789` intact post body-append. AC-4: `TestIndMutationSeq/archive-flat` + `TestNativeUnknownFieldPreservation/archive` (tracker fields + `archived:` survive the move). AC-5: zero tracker-name literals (kata/linear/jira/github) and zero `issue` special-casing in non-test source (grep exit 1); `updateFrontmatter` (mutate.go:38) is a line-rewriter that re-emits every non-matching frontmatter line verbatim — the A1 risk (fixed known-field struct) is structurally absent. AC-6: DECISION cites the real roadmap out-of-scope line (bootstrap-roadmap.md:20) and the real ownership principles (state-behavior-extension.md:84-85).
- DONE: Run go test ./... and go test ./... -race, gofmt -l, go vet — capture REAL exit codes (capture $?; never pipe to tail).
  `go test ./...` exit 0 (cli ok, status ok 12.8s, integration ok; cmd/spacedock no test files); `go test ./... -race` exit 0 (all ok); `gofmt -l .` exit 0 + EMPTY output (0 bytes); `go vet ./...` exit 0 + EMPTY output. Raw per-package `ok` lines captured via `rtk proxy` to bypass the test-output summarizer; exit codes captured from `$?`, never piped.
- DONE: Confirm the AC-3 test is load-bearing: flip the asserted issue/source value and confirm it fails.
  Flipped `issue: kata:task-xyz789` -> `issue: kata:task-WRONG` in native_mutation_test.go:222 -> test FAILED exit 1 at line 224, failure dump showing the REAL preserved frontmatter (`source: kata`, `issue: kata:task-xyz789`); reverted, re-ran, PASS exit 0, worktree clean. Test asserts on real preserved bytes, not a tautology.

### Summary

All six ACs independently reproduced; recommendation PASSED. The deliverable is a design checkpoint (decision + preserve-through-lifecycle contract + folded fixture proof), not a feature, and that is what was produced. The load-bearing A1 risk — a mutator that re-emits only known fields — is structurally absent: `updateFrontmatter` rewrites matching frontmatter lines in place and re-joins every other line verbatim, so `issue`/`source` survive `--set` and `--archive` byte-for-byte, proven both by the independent-oracle parity suite and the dedicated preservation tests. The new AC-3 test is load-bearing (confirmed by a flip-and-fail). AC-5/AC-6 hold on static review against real cited source (no tracker branching in code; DECISION citations verified verbatim in the roadmap and state-behavior extension). All four gates clean with real captured exit codes.
