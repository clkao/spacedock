...
completed: 2026-05-31T04:10:28Z
verdict: PASSED
score: "0.30"
worktree: 
issue:
archived: 2026-05-31T04:10:28Z
---
...
Acted on the validation's two follow-ups (captain decision via team-lead): migrate `brews:` → `homebrew_casks:` and gitignore `dist/`.

- DONE: migrate `.goreleaser.yaml` `brews:` → `homebrew_casks:`; `goreleaser check` now exits 0.
  Commit `27c529d` on `spacedock-ensign/release-pipeline`. `brews` is hard-deprecated at goreleaser v2.16 (the validation's exit-2 finding). Replaced with `homebrew_casks:` — dropped the `install`/`test` stanzas and the also-deprecated `homebrew_casks.binary` field (each verified deprecated/unsupported by probing `goreleaser check`); casks auto-detect the bare `spacedock` binary from the archive root. The `agent-safehouse.dev` caveat, `repository: spacedock-dev/homebrew-tap`, and `token: HOMEBREW_TAP_TOKEN` carry over. PROOF (goreleaser v2.16.0): `goreleaser check` → exit 0 ("1 configuration file(s) validated"), the deprecation that gave exit 2 is gone; `goreleaser release --snapshot --clean` → exit 0, emits both darwin tarballs + checksums.txt + generated `dist/homebrew/Casks/spacedock.rb` (cask DSL, `binary "spacedock"`, per-arch url `spacedock_#{version}_darwin_{arm64,amd64}.tar.gz`, sha256 from checksums, `agent-safehouse.dev` caveat); the snapshot arm64 binary RUN → `spacedock 0.0.1-snapshot-60989ed (contract 1)`.
- DONE: gitignore `dist/` (goreleaser build output, never tracked).
  Commit `cd03237`: added `dist/` to `.gitignore`. Verified by RUNNING `goreleaser release --snapshot --clean` then `git status` → only ` M .gitignore`, no untracked `dist/` (`git check-ignore` confirms `dist/homebrew/Casks/spacedock.rb` is ignored).
- DONE: NO hand-authored bootstrap cask; rg's `dist/homebrew/Formula/spacedock.rb` + README untouched.
  Per captain decision: the `homebrew_casks:` block IS the cask source of truth — the first `goreleaser release` generates and pushes the cask to the tap, so a pre-release hand-authored cask (whose urls would 404) buys nothing. rg's superseded `Formula/spacedock.rb` + README on main are left for the FO to `git rm` at the merge boundary (outside my worktree scope; my contract forbids writing to main). The B↔C "keep formula in sync" contract is now MOOT: goreleaser generates BOTH the darwin binary AND the cask from the same archive, so the url shape + sha256 + payload are consistent by construction, not by a hand-maintained agreement.
- DONE: gates green (config + gitignore only).
  `go test ./...` exit 0 (470 passed / 8 pkgs).

### Summary

Migrated the release config off the hard-deprecated `brews:` to `homebrew_casks:` (commit `27c529d`) — this directly closes the validation's blocking-adjacent finding that `goreleaser check` exited 2. Behavioral proof with the now-installed goreleaser v2.16.0: `goreleaser check` exits 0 and `goreleaser release --snapshot --clean` exits 0, producing both darwin tarballs, `checksums.txt`, and a generated cask carrying the `agent-safehouse.dev` caveat; the snapshot binary RUN reports the goreleaser-stamped version. Also gitignored goreleaser's `dist/` build output (commit `cd03237`, verified no untracked `dist/` after a snapshot run). Per the captain's decision relayed by team-lead, I did NOT hand-author a bootstrap cask (the `homebrew_casks:` block is the source of truth; the first release generates + pushes it) and did NOT touch rg's superseded `dist/homebrew/Formula` + README (the FO `git rm`s them at the merge boundary). The B↔C formula-sync contract is moot — goreleaser generates the binary and the cask together, consistent by construction. `go test ./...` stays green (exit 0). The first-release `v0.19.0` tag + `HOMEBREW_TAP_TOKEN` secret remain captain/CI-only steps.

## Stage Report: validation (cycle 2)

Re-validation after the `brews:`→`homebrew_casks:` migration (cycles 3-4). goreleaser v2.16.0 IS installed here — I reproduced the cask proofs by RUNNING the tool + the binary, not by grep. Worktree HEAD `cd03237` (0 tags, `git describe --tags --always` = `cd03237`).

- DONE: Independently reproduce the cask migration (`goreleaser check` exit 0; `goreleaser release --snapshot --clean` exit 0 generating a cask; RUN the snapshot binary → `--version` reports the git-describe stamp).
  `goreleaser check` → exit 0 ("1 configuration file(s) validated") — the brews hard-deprecation that gave exit 2 in cycle-1 validation is GONE. `goreleaser release --snapshot --clean` → exit 0 ("release succeeded"): built both darwin targets, wrote `dist/spacedock_0.0.1-snapshot-cd03237_darwin_{arm64,amd64}.tar.gz`, `dist/checksums.txt`, and `dist/homebrew/Casks/spacedock.rb`. The cask is real Cask DSL: `cask "spacedock" do`, `binary "spacedock"` stanza, per-arch `url "…/spacedock_#{version}_darwin_{amd64,arm64}.tar.gz"`, per-arch `sha256` matching `checksums.txt`, and the `https://agent-safehouse.dev` caveat. RAN the goreleaser-built arm64 binary → `spacedock 0.0.1-snapshot-cd03237 (contract 1)` — the ldflags stamp (goreleaser's git-describe-derived snapshot version) flows through end-to-end, NOT the `0.19.0` default. F9 proven through the real release tool.
- DONE: Confirm `dist/` is gitignored (snapshot leaves NO untracked dist/); homebrew_casks carries repository=spacedock-dev/homebrew-tap + token=HOMEBREW_TAP_TOKEN + name_template making the cask url match the tarballs by construction; `var Version = "0.19.0"` holds (TestVersion + TestVersionContractToken pass).
  After the snapshot run `git status --short` = empty (no untracked `dist/`); `git check-ignore dist/homebrew/Casks/spacedock.rb` confirms it's ignored (`.gitignore:7` = `dist/`). Config: `homebrew_casks.repository` = owner `spacedock-dev` / name `homebrew-tap`, `token: {{ .Env.HOMEBREW_TAP_TOKEN }}`. The archive `name_template` `{{.ProjectName}}_{{.Version}}_{{.Os}}_{{.Arch}}` and the cask url filename `spacedock_#{version}_darwin_{arch}.tar.gz` are the SAME string by construction — and I verified the cask's two sha256 values equal `checksums.txt`, which equals my independent `shasum -a 256` of the tarballs (arm64 `f615ea60…`, amd64 `258b9bd3…`). `var Version = "0.19.0"` at internal/cli/cli.go:23; `TestVersion` + `TestVersionContractToken` → both `--- PASS`. Negative control: `-X main.Version=v9.9.9-trap` build RAN → still `0.19.0` (the F9 trap silently no-ops, so the internal/cli path is load-bearing).
- DONE: Gates green with REAL captured exit codes (go test, -race, gofmt, go vet); the live `brew audit --strict --cask` is captain/CI post-first-release — confirm honestly deferred, not faked.
  `go test ./...` exit 0 (470 passed / 8 pkgs); `go test -race ./...` exit 0 (470 passed, no DATA RACE); `gofmt -l .` exit 0 (0-line file list, truly empty); `go vet ./...` exit 0 (no issues). Deferral is HONEST: the entity has ZERO `brew audit` claims (grep → no hits) and none was fabricated. A live `brew audit --strict --cask` genuinely cannot run pre-release — the cask url points at `…/releases/download/v0.0.0/spacedock_…tar.gz`, which only exists after the first real `goreleaser release` on a real tag; it is correctly captain/CI-only.

### Summary

RECOMMENDATION: **PASSED**. The `brews:`→`homebrew_casks:` migration is reproduced by RUNNING goreleaser v2.16.0, not grep: `goreleaser check` exits 0 (the cycle-1 exit-2 deprecation is gone) and `goreleaser release --snapshot --clean` exits 0, generating a real Cask DSL (`binary "spacedock"`, per-arch `spacedock_#{version}_darwin_{arch}.tar.gz` url, per-arch sha256 from checksums.txt, `agent-safehouse.dev` caveat). The cask url filenames equal the archive name_template by construction, and the cask sha256s equal checksums.txt equal an independent `shasum -a 256` of the tarballs. The F9 stamp flows end-to-end through the goreleaser-built binary (`spacedock 0.0.1-snapshot-cd03237`, NOT the `0.19.0` default; the `-X main.Version` trap is a confirmed silent no-op). `dist/` is gitignored (empty `git status` + `git check-ignore` after a snapshot run). All gates green with real exit codes (test/race/gofmt/vet all 0); TestVersion + TestVersionContractToken pass. The `brew audit --strict --cask` deferral is honest (zero audit claims in the entity; it cannot run until release assets exist at the v* tag url). The first-release `v0.19.0` tag + `HOMEBREW_TAP_TOKEN` secret + the FO's `git rm` of rg's superseded `dist/homebrew/Formula` + README at merge remain genuine captain/CI/FO steps, none blocking the merge.
