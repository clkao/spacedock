...
- It relies on **exit codes** (0 success, 1 domain error, 2 usage) to gate the loop, and on the **mutation guards** in `--set`/`--archive` (mod-block, merge-hook, terminal-transition invariants) plus the **validation messages** that gate state trust.
...
**AC-5 - Validation reproduces the current failure set and messages for the documented error classes.**
...
**AC-6 - `--new` atomically mints an id and writes a valid entity, so a seed never exists id-less, and the id-strictness rule is consistent across read, mutate, and create.**
